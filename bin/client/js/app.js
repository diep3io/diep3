...
/***/function (module, exports) {
	...
	function getZoneColor(cell, real) {
		switch (cell) {
			...
			case 'dom':
				return color.yellow;
			case 'nest':
				return real ? color.guiwhite : color.lavender;
			default:
	...
	var _gui = {
		getStatNames: function getStatNames(num) {
			switch (num) {
				case 1:
					return ['Body Damage', 'Max Health', 'Bullet Speed', 'Bullet Health', 'Bullet Penetration', 'Bullet Damage', 'Engine Acceleration', 'Movement Speed', 'Health Regeneration', 'Resistance'];
				case 2:
					return ['Body Damage', 'Max Health', 'Drone Speed', 'Drone Health', 'Drone Penetration', 'Drone Damage', 'Respawn Rate', 'Movement Speed', 'Health Regeneration', 'Resistance'];
				case 3:
					return ['Body Damage', 'Max Health', 'Drone Speed', 'Drone Health', 'Drone Penetration', 'Drone Damage', 'Max Drone Count', 'Movement Speed', 'Health Regeneration', 'Resistance'];
				case 4:
					return ['Body Damage', 'Max Health', 'Swarm Speed', 'Swarm Health', 'Swarm Penetration', 'Swarm Damage', 'Reload', 'Movement Speed', 'Health Regeneration', 'Health Capacity'];
				case 5:
					return ['Body Damage', 'Max Health', 'Placement Speed', 'Trap Health', 'Trap Penetration', 'Trap Damage', 'Reload', 'Movement Speed', 'Health Regeneration', 'Resistance'];
				case 6:
					return ['Body Damage', 'Max Health', 'Weapon Speed', 'Weapon Health', 'Weapon Penetration', 'Weapon Damage', 'Reload', 'Movement Speed', 'Health Regeneration', 'Resistance'];
				default:
					return ['Body Damage', 'Max Health', 'Bullet Speed', 'Bullet Health', 'Bullet Penetration', 'Bullet Damage', 'Reload', 'Movement Speed', 'Health Regeneration', 'Resistance'];
			}
		},
		skills: [{
			amount: 0,
	...
	var serverName = 'Open TDM Public';
					...
					case 'S':
						{
							...
							if (sync.length < 10) {
								...
								global.message = "Connecting... - " + sync.length + "/10...";
							} else {
	...
	var gameDraw = function () {
		...
		// Text objects
		var text = {
			skillNames: [TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj()],
			...
			upgradeNames: [TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj(), TextObj()],
		...
		return function (ratio) {
...

			{
				...
				roomSetup.forEach(function (row) {
					var j = 0;
					row.forEach(function (cell) {
						ctx.fillStyle = getZoneColor(cell, true);
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' visitors defeated' : '') : 'Good Job!') + '.';
		};
		var getDeath = function getDeath() {
			...
			if (global.finalKillers.length) {
				...
				txt += 'You died to your own dumbness';
			}
			return txt;
		};
		return function () {
			...
			text.taunt.draw('', x, y - 80, 11, color.guiwhite, 'center');
			text.level.draw('level = ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name + '.', x - 170, y - 30, 24, color.guiwhite);
			text.score.draw('Final score:: ' + formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.guiwhite);
			text.time.draw('Stayed alive for ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.', x - 170, y + 55, 16, color.guiwhite);
	...
	var gameDrawBeforeStart = function () {
		...
		return function () {
			...
			text.connecting.draw('Entering the game room', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite, 'center');
			text.message.draw(global.message, global.screenWidth / 2, global.screenHeight / 2 + 30, 15, color.guiwhite, 'center');
		};
	}();
